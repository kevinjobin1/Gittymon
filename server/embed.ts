...
export function generateSvgCard(username: string, palette?: string, provider?: string): string {
  ...
  const spriteResult = buildServerSpriteGrid(spriteSeed, 0, paletteOverride);
...
export function generateGifCard(username: string, palette?: string, provider?: string): Buffer {
  ...
  for (let f = 0; f < NUM_FRAMES; f++) {
    ...
    // Sprite with bounce (48×48 at 2px scale)
    ...
    // REPOS/FOLLOWER
    canvas.drawText(`R/F:${githubMockData.public_repos}/${githubMockData.followers}`, infoX, 20, 1);
