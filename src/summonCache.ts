...
export async function addToSummonCache(kv: KVNamespace, username: string, resultMon: RoastMon): Promise<void> {
...
export async function lookupSummonCache(kv: KVNamespace, username: string): Promise<RoastMon | null> {
  const cache = await loadSummonCache(kv);
  const cached = cache.find(e => e.username.toLowerCase() === username.toLowerCase());
