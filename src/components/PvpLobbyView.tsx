...
export function PvpLobbyView({
  ...
  return (
    <div className="flex-1 flex flex-col justify-between p-1 px-1.5 md:px-2.5 md:py-1.5 text-[#1a1a1a] select-none font-mono">
      {/* Visual Header */}
      <div className="flex justify-between items-center border-b-2 border-[#1a1a1a] pb-1 font-bold text-[9px] sm:text-[10px]">
      ...
      <div className="flex-1 flex flex-col justify-center py-2 space-y-2">
        ...
        <div className="border border-neutral-300 bg-neutral-100 rounded p-1.5 text-center shadow-[1px_1px_0px_white_inset]">
          <div className="text-[10px] sm:text-[12px] font-extrabold text-neutral-800 tracking-tight">
            ONLINE TRAINERS: {onlineCount}
          </div>
          
          {idlePlayers.length > 0 ? (
            <div className="mt-1 text-[7px] sm:text-[8px] text-gray-400 font-bold max-h-5.5 overflow-hidden truncate">
              IDLE: {idlePlayers.slice(0, 3).join(', ')}
            </div>
          ) : (
            <div className="mt-1 text-[7.5px] sm:text-[8.5px] text-gray-500">
        ...
        <div className="flex-1 flex flex-col items-center justify-center border-2 border-dashed border-[#1a1a1a] bg-neutral-50 rounded p-2 text-center select-none min-h-[clamp(90px,18vh,150px)]">
          {isSearching ? (
            <div className="space-y-1.5">
              ...
              <div className="text-[9px] sm:text-[10px] font-bold text-neutral-800 leading-none">
                FINDING OPPONENT{searchDots}
              </div>
              <div className="text-[6px] sm:text-[7px] text-gray-400 font-bold px-1 uppercase leading-tight">
                Allocating gladiator. Bots standby if alone...
              </div>
            </div>
          ) : (
            <div className="space-y-1">
              <div className="text-[12px] sm:text-[15px] font-black text-[#7f001c] shrink-0 animate-bounce leading-none">READY!</div>
              <div className="text-[8px] sm:text-[9px] text-gray-500 px-1 font-semibold leading-tight">
                Enter matchmaking to battle real owners online!
              </div>

              {searchError && (
                <div className="text-[7px] sm:text-[8px] text-red-500 font-bold px-1 bg-red-50 rounded mt-1 border border-red-200">
      ...
      <div className="space-y-1 font-mono">
        <button
          ...
          className={`w-full py-[clamp(3px,1.5vh,8px)] border-2 border-[#1a1a1a] rounded text-[8.5px] sm:text-[10px] font-black shadow-[1.5px_1.5px_0px_#1a1a1a] transition-all active:translate-y-0.5 ${
            isSearching
        ...
        {!isSearching && (
          <button
            onClick={onBack}
            className="w-full py-0.5 border border-dashed border-gray-400 rounded text-[7px] sm:text-[8px] font-bold text-gray-600 hover:bg-neutral-200 transition-colors"
          >
            SELECT = BACK TO HUB
          </button>
        )}
