...
interface HubViewProps {
  ...
  registerDirectionHandler: (
    handler: ((dir: "UP" | "DOWN" | "LEFT" | "RIGHT") => void) | null,
  ) => void;
...
export function HubView({
  ...
  const OPTIONS = [
    {
      id: "STATS",
      label: "1. GITTYMON STATS",
      desc: "Inspect level, moves & stats",
    },
    {
      id: "SINGLE_FIGHT",
      label: "2. LOCAL GLADIATOR",
      desc: "Battle classic offline bugs",
    },
    {
      id: "PVP_LOBBY",
      label: "3. ONLINE ARENA",
      desc: `PVP over WebSocket [Live!]`,
    },
    {
      id: "AI_BOSS",
      label: "4. AI GLITCH BOSS",
      desc: "Savage live AI Gym Leader",
    },
    {
      id: "LEADERBOARD",
      label: "5. GLOBAL RANKS",
      desc: "Check high scores list",
    },
    {
      id: "EXPORT_EMBED",
      label: "6. EXPORT BADGE",
  ...
  return (
    <div className="flex-1 flex flex-col justify-between p-1 px-1.5 text-[#1a1a1a] select-none font-mono">
      ...
      <div className="flex justify-between items-center border-b-2 border-[#1a1a1a] pb-1.5 font-bold text-[9px] tracking-tight">
        ...
        <span className="text-emerald-800 animate-pulse text-[8px]">
      ...
      <div className="flex items-center space-x-2 bg-neutral-100 border border-neutral-300 rounded p-1 my-1 shadow-[1px_1px_0px_rgba(0,0,0,0.1)]">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="text-[9px] font-bold text-[#7f001c] truncate">
            {mon.name.toUpperCase()}
          </div>
          <div className="text-[7.5px] text-gray-600 leading-none">
            LV {mon.level} • {mon.type.toUpperCase()}
          </div>
        </div>
      </div>
