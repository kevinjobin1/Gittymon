...
export function BackgroundMap() {
    ...
    const handleGlobalClick = (e: MouseEvent) => {
      ...
      GittymonsRef.current.forEach((mon) => {
        ...
        if (dist < 240) {
          ...
          mon.targetX =
            mon.x + (dx / scaleDir) * 200 + (Math.random() - 0.5) * 80;
          mon.targetY =
            mon.y + (dy / scaleDir) * 200 + (Math.random() - 0.5) * 80;
          ...
          mon.clickReactionText =
            reactPhrases[Math.floor(Math.random() * reactPhrases.length)];
  ...
  useEffect(() => {
    ...
    if (GittymonsRef.current.length === 0) {
      const types: ("trex" | "slime" | "octo" | "bat")[] = [
        "trex",
        "slime",
        "octo",
        "bat",
      ...
      Gittymons.forEach((mon) => {
        ...
        if (mon.state === "panic") {
          ...
          if (mon.idleTimer <= 0) {
            ...
            if (
              Math.abs(mon.targetX - devCenterX) < 220 &&
              Math.abs(mon.targetY - devCenterY) < 330
            ) {
