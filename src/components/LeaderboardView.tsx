...
export function LeaderboardView({
  ...
  // Hook D-pad to scroll down lists if long
  useEffect(() => {
    const handleDirection = (dir: 'UP' | 'DOWN' | 'LEFT' | 'RIGHT') => {
      if (dir === 'UP') {
        ...
        playRetroSound('beep');
        const maxScroll = Math.max(0, entries.length - 4);
  ...
  const visibleEntries = entries.slice(scrollIndex, scrollIndex + 4);
  ...
  return (
    <div className="flex-1 flex flex-col justify-between p-1 px-1.5 text-[#1a1a1a] select-none font-mono">
      ...
      <div className="flex justify-between items-center border-b-2 border-[#1a1a1a] pb-1 font-bold text-[9px]">
        ...
        <span className="text-[#7f001c]">TOP SUMMONS</span>
      </div>

      <div className="flex-1 flex flex-col justify-center py-1">
        {loading ? (
          <div className="flex-1 flex flex-col items-center justify-center space-y-2 text-[9px] text-gray-400">
          ...
          <div className="space-y-1.5 flex-1 flex flex-col justify-start pt-1">
            {visibleEntries.map((entity, relativeIdx) => {
              const actualIdx = scrollIndex + relativeIdx;
            ...
            {/* Scroll Indicator */}
            {entries.length > 4 && (
              <div className="text-center text-[7px] text-gray-400 font-bold leading-none animate-bounce pt-0.5">
                {scrollIndex + 4 < entries.length ? '▼ MORE PLAYERS ▼' : '▲ SCROLLED TO END ▲'}
              </div>
            )}
          </div>
        )}
